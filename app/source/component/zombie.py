...
class PortraitZombie(NormalZombie):
    """Photo-faced normal-strength enemy with complete custom animation sets."""

    ASSET_NAME = c.PORTRAIT_ZOMBIE

    def __init__(self, x, y, head_group, can_swim=False):
        Zombie.__init__(
            self, x, y, self.ASSET_NAME, head_group, can_swim=can_swim,
        )
        self.walk_animate_interval = 65
        self.attack_animate_interval = 45
        self.losthead_animate_interval = 100
        self.die_animate_interval = 65
        self.boomDie_animate_interval = 80
        self.animate_interval = self.walk_animate_interval

    def loadImages(self):
        for attr, suffix in (
            ('walk_frames', ''), ('attack_frames', 'Attack'),
            ('losthead_walk_frames', 'LostHead'),
            ('losthead_attack_frames', 'LostHeadAttack'),
            ('die_frames', 'Die'), ('boomdie_frames', 'BoomDie'),
            ('idle_frames', 'Idle'),
        ):
            setattr(self, attr, tool.GFX[self.ASSET_NAME + suffix])
        self.frames = self.walk_frames
        self.swim_frames = self.walk_frames

    def setLostHead(self):
        # Keep the photographed face intact: the normal low-health mechanics
        # become a dazed/staggering phase instead of spawning a generic head.
        self.losthead_timer = self.current_time
        self.losthead = True
        self.animate_interval = self.losthead_animate_interval
    ...
    def changeFrames(self, frames):
        super().changeFrames(frames)
        self.animate_timer = getattr(self, 'current_time', 0)

    def animation(self):
        if self.state != c.FREEZE:
            ratio = self.getAttackTimeRatio() if self.state == c.ATTACK else self.getTimeRatio()
            interval = self.animate_interval * ratio
            steps = int((self.current_time - self.animate_timer) // interval)
            if steps > 0:
                self.frame_index += steps
                self.animate_timer += steps * interval
                if self.state == c.DIE and self.frame_index >= self.frame_num:
                    self.kill()
                    return
                self.frame_index %= self.frame_num
        # Never tint or change alpha on shared cached frames.
        image = self.frames[self.frame_index].copy()
        if self.swimming and self.state != c.DIE:
            # Submerge legs without moving the collision anchor or changing
            # the photographed face. Reuse frame phase so ice freezes ripples.
            width, height = image.get_size()
            waterline = round(height * 0.76)
            image.fill((0, 0, 0, 0), (0, waterline, width, height - waterline))
            swell = round(math.sin(self.frame_index * math.tau / self.frame_num) * 3)
            pg.draw.ellipse(
                image, (125, 218, 248, 180),
                (width // 5 - swell, waterline - 5, width * 3 // 5 + swell * 2, 12), 2,
            )
        if self.is_hypno:
            image = pg.transform.flip(image, True, False)
        age = self.current_time - self.hit_timer
        if 0 <= age < 200 and self.state != c.DIE:
            image.fill((45, 25, 18, 0), special_flags=pg.BLEND_RGBA_ADD)
            recoil = pg.Surface(image.get_size(), pg.SRCALPHA)
            recoil.blit(image, (round(math.sin(age / 200 * math.pi) * 3), 0))
            image = recoil
        if self.state == c.FREEZE:
            image.fill((130, 190, 255, 255), special_flags=pg.BLEND_RGBA_MULT)
        elif self.ice_slow_ratio > 1:
            image.fill((195, 220, 255, 255), special_flags=pg.BLEND_RGBA_MULT)
        self.image = image
        self.mask = pg.mask.from_surface(image)


class ElitePortraitZombie(PortraitZombie):
    """Rare, tougher bruiser with a complete generated portrait animation set."""

    ASSET_NAME = c.ELITE_PORTRAIT_ZOMBIE

    def __init__(self, x, y, head_group, can_swim=False):
        super().__init__(x, y, head_group, can_swim=can_swim)
        self.health = c.ELITE_PORTRAIT_HEALTH
        self.damage = c.ELITE_PORTRAIT_DAMAGE
