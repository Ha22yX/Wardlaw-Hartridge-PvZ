...
from ..component.face_shovel import ShovelChew
from ..component.portrait_dave import FirstLawnGuide, StageIntro
from ..component.custom_campaign import CampaignBattle, CardRewards, CardTooltip, cards_for
from ..component.impact_audio import ImpactAudio
from ..component import plant_layout
from ..component.pvz_ui import draw_button
...
class Level(tool.State):
    ...
    def startup(self, current_time, persist):
        old_guide = getattr(self, 'first_lawn_guide', None)
        if old_guide:
            old_guide.voice.stop()
        self.game_info = persist
        self.persist = self.game_info
        self.game_info[c.CURRENT_TIME] = current_time
        self.current_time = current_time
        self.speed_multiplier = 1
        self.speed_elapsed = 0
        self.impact_audio = ImpactAudio()
        self.shortcut_message = ''
        self.shortcut_until = 0
        self.shortcut_font = pg.font.Font(c.FONT_PATH, 19)
        self.first_lawn_guide = None
        self.campaign = None
        self.card_rewards = None
        self.card_tooltip = CardTooltip()
        self.guide_elapsed = 0
        self.guide_last_time = current_time
        self.guide_enabled = persist[c.GAME_MODE] == c.MODE_ADVENTURE
        ...
        if self.guide_enabled:
            stage = self.map_data['custom_stage']
            self.first_lawn_guide = (FirstLawnGuide(self, current_time) if stage == 1
                                     else StageIntro(self, current_time, stage))

    def shortcutTip(self, message):
        self.shortcut_message = message
        self.shortcut_until = pg.time.get_ticks() + 1800

    def handle_key(self, event):
        """Single-press gameplay shortcuts; never grant a completion for jumping."""
        if self.done or self.card_rewards is not None or getattr(event, 'repeat', False):
            return
        if getattr(event, 'mod', 0) & (pg.KMOD_CTRL | pg.KMOD_ALT | pg.KMOD_META):
            return
        key = event.key
        if key in (pg.K_9, pg.K_KP9):
            if self.bar_type != c.CHOOSEBAR_STATIC:
                self.shortcutTip('传送带模式不使用阳光')
            elif self.state == c.CHOOSE:
                # Copy per-level configuration; do not change the global map defaults.
                self.map_data = self.map_data.copy()
                value = min(9990, self.map_data[c.INIT_SUN_NAME] + 100)
                self.map_data[c.INIT_SUN_NAME] = value
                self.panel.value_image = menubar.getSunValueImage(value)
                self.shortcutTip(f'开局阳光：{value}')
            elif self.state == c.PLAY:
                before = self.menubar.sun_value
                self.menubar.increaseSunValue(100)
                self.menubar.update(self.current_time)
                self.shortcutTip(f'阳光 +{self.menubar.sun_value - before}')
            return True
        if key in (pg.K_0, pg.K_KP0):
            if self.state == c.PLAY and self.bar_type == c.CHOOSEBAR_STATIC:
                ...
                self.menubar.update(self.current_time)
                self.shortcutTip('所有植物卡冷却已清空')
            else:
                self.shortcutTip('当前没有需要清除的植物卡冷却')
            return True
        if key in (pg.K_MINUS, pg.K_KP_MINUS):
            delta = -1
        elif key in (pg.K_PLUS, pg.K_EQUALS, pg.K_KP_PLUS):
            delta = 1
        else:
            return
        adventure = self.game_info[c.GAME_MODE] == c.MODE_ADVENTURE
        progress_key = c.LEVEL_NUM if adventure else c.LITTLEGAME_NUM
        last = (map.TOTAL_LEVEL if adventure else map.TOTAL_LITTLE_GAME) - 1
        current = self.game_info[progress_key]
        target = max(1, min(last, current + delta))
        if target == current:
            self.shortcutTip('已经是第一关' if delta < 0 else '已经是最后一关')
            return True
    ...
    def drawShortcutTip(self, surface):
        if self.shortcut_message and pg.time.get_ticks() < self.shortcut_until:
            label = self.shortcut_font.render(self.shortcut_message, True, (255, 244, 199))
            rect = label.get_rect(center=(c.SCREEN_WIDTH // 2, 163))
            pg.draw.rect(surface, (38, 59, 40), rect.inflate(26, 16), border_radius=9)
            surface.blit(label, rect)

    def loadMap(self):
        # 冒险模式
        if self.game_info[c.GAME_MODE] == c.MODE_ADVENTURE:
            if 1 <= self.game_info[c.LEVEL_NUM] < map.TOTAL_LEVEL:
    ...
    def setupGroups(self):
        self.portrait_spawn_count = 0
        self.sun_group = pg.sprite.Group()
        self.head_group = pg.sprite.Group()
        self.shovel_effects = pg.sprite.Group()
        self.heal_effects = pg.sprite.Group()
        self.last_shovel_use = None
                ...
                if not self.grave_zombie_created:
                    if current_time - self.wave_time > 1500:
                        for item in self.grave_set:
                            ...
                            self.spawnPortraitZombie(item_x, item_y, item[1])
                ...
                if not self.created_zombie_from_pool:
                    if current_time - self.wave_time > 1500:
                        for i in range(3):
                            ...
                            self.spawnPortraitZombie(item_x, item_y, map_y)
    ...
    def setupCars(self):
        self.cars = []
        for i in range(self.map_y_len):
            if ('custom_stage' in self.map_data
                    and self.map.map[i][0][c.MAP_PLOT_TYPE] == c.MAP_UNAVAILABLE):
    ...
    def update(self, surface, current_time, mouse_pos, mouse_click):
        if self.card_rewards is not None:
            # Reward UI owns input; freeze the board and never plant on a claim click.
            if self.card_rewards.update(current_time, mouse_pos, mouse_click):
                self.completeCampaignStage()
            self.draw(surface)
            self.card_rewards.draw(surface)
            return
        guide = self.first_lawn_guide
        previous_time = self.current_time
        elapsed = max(0, current_time - self.guide_last_time)
        frame_speed = self.speed_multiplier if self.canToggleSpeed() else 1
        self.speed_elapsed += elapsed * (frame_speed - 1)
        if guide and guide.active:
            self.guide_elapsed += elapsed
        self.guide_last_time = current_time
        self.current_time = self.game_info[c.CURRENT_TIME] = self.gameTime(
            current_time - self.guide_elapsed + self.speed_elapsed
        )
        if guide and guide.active:
            guide.update(self, current_time, mouse_pos, mouse_click)
            self.draw(surface)
            if guide.active:
                guide.draw(surface, self)
            self.updateCardTooltip(surface, current_time)
            self.drawShortcutTip(surface)
            self.impact_audio.set_volume(self.game_info[c.SOUND_VOLUME])
            return
        if self.state == c.CHOOSE:
            self.choose(mouse_pos, mouse_click)
        elif self.state == c.PLAY:
            if (self.canToggleSpeed() and mouse_pos and mouse_click[0]
                    and self.speed_button_rect.collidepoint(mouse_pos)):
                self.speed_multiplier = 2 if self.speed_multiplier == 1 else 1
                c.SOUND_BUTTON_CLICK.play()
                mouse_pos, mouse_click = None, [False, False]
            # Two ordinary simulation steps accelerate even legacy per-frame
            # movement (peas/cars) without changing their damage or geometry.
            # Deliver a click only once, on the final step.
            target_time = self.current_time
            for step in range(1, frame_speed + 1):
                self.current_time = self.game_info[c.CURRENT_TIME] = (
                    round(previous_time + (target_time - previous_time) * step / frame_speed))
                self.play(mouse_pos if step == frame_speed else None,
                          mouse_click if step == frame_speed else [False, False])
                if self.done or self.card_rewards is not None:
                    break
    ...
    def canToggleSpeed(self):
        guide = self.first_lawn_guide
        return (self.state == c.PLAY and not self.pause and not self.show_game_menu
                and self.card_rewards is None and not (guide and guide.active))

    def drawSpeedButton(self, surface):
        if self.canToggleSpeed():
            draw_button(surface, self.speed_button_rect,
                        '关闭2倍速' if self.speed_multiplier == 2 else '开启2倍速',
                        self.speed_button_font)

    def updateCardTooltip(self, surface, now):
        cards = getattr(getattr(self, 'menubar', None), 'card_list', [])
        enabled = (self.state == c.PLAY and not self.show_game_menu
                   and not self.drag_plant and not self.drag_shovel
                   and pg.mouse.get_focused())
        self.card_tooltip.update(cards, pg.mouse.get_pos(), pg.time.get_ticks(), enabled)
        self.card_tooltip.draw(surface)

    def completeCampaignStage(self):
        if self.done:
            return
        stage = self.map_data['custom_stage']
        self.game_info[c.CUSTOM_CAMPAIGN_CLEARED] = max(
            self.game_info.get(c.CUSTOM_CAMPAIGN_CLEARED, 0), stage)
        self.game_info[c.LEVEL_NUM] = min(5, stage + 1)
        self.saveUserData()
        self.next = c.LEVEL
        self.done = True

    def cleanup(self):
        if self.first_lawn_guide:
            self.first_lawn_guide.voice.stop()
    ...
    def initState(self):
        ...
        if self.bar_type == c.CHOOSEBAR_STATIC:
            if self.guide_enabled:
                self.initPlay([c.PLANT_CARD_INDEX[name]
                               for name in cards_for(self.map_data['custom_stage'])])
            else:
                self.initChoose()
        else:
    ...
    def initPlay(self, card_list):
        ...
        self.setupCars()
        self.campaign = (CampaignBattle(self) if 'custom_stage' in self.map_data else None)
        if self.campaign:
            self.wave_num = 0
            self.wave_time = 0
            self.wave_zombies = []
        ...
        if self.has_shovel:
            ...
            self.shovel_open = tool.GFX['shovel_open_cursor']
    ...
    def setupLittleMenu(self):
        ...
        self.speed_button_rect = pg.Rect(690, self.little_menu_rect.bottom + 6, 110, 30)
        self.speed_button_font = pg.font.Font(c.FONT_PATH, 17)
    ...
    def shovelRemovePlant(self, mouse_pos):
        ...
        if not self.map.isValid(map_x, map_y):
            return
        for i in self.plant_groups[map_y]:
            if plant_layout.visual_rect(i.name, i.image, i.rect).collidepoint(x, y):
                ...
                if len(self.map.map[map_y][map_x][c.MAP_PLANT]) >= 2:
                    if c.LILYPAD in self.map.map[map_y][map_x][c.MAP_PLANT]:
                        ...
                        if i.name == '花盆（未实现）':
                            continue
                # Snapshot the plant before removing it from gameplay, so only
                # its visual is swallowed; tile cleanup still happens once.
                effect = ShovelChew(tool.GFX, i, self.current_time)
                self.killPlant(i, shovel=True)
                self.shovel_effects.add(effect)
                self.last_shovel_use = self.current_time
    ...
    def play(self, mouse_pos, mouse_click):
        ...
        if self.campaign:
            self.campaign.update(self, self.current_time)
        elif self.map_data[c.SPAWN_ZOMBIES] == c.SPAWN_ZOMBIES_LIST:
            # 旧僵尸生成方式
        ...
        self.shovel_effects.update(self.game_info)
        self.heal_effects.update(self.game_info)
        ...
        if self.produce_sun:
            ...
            if (self.current_time - self.sun_timer) > min(
                ...
                if self.campaign:
                    map_y = random.choice(self.campaign.rows)
    ...
    def createZombie(self, name, map_y=None):
        # Keep original type for land/water lane selection, then replace it
        # with the portrait enemy without changing wave counts or timing.
        ...
        self.spawnPortraitZombie(
            c.ZOMBIE_START_X + random.randint(-20, 20) + huge_wave_move,
            y, map_y,
        )

    def spawnPortraitZombie(self, x, y, map_y):
        """Shared spawn path for waves, tombstones and pool ambushes."""
        can_swim = (
            self.map_data[c.BACKGROUND_TYPE] in c.POOL_EQUIPPED_BACKGROUNDS
            and map_y in (2, 3)
        )
        self.portrait_spawn_count += 1
        enemy_type = (zombie.ElitePortraitZombie
                      if self.portrait_spawn_count % c.ELITE_PORTRAIT_SPAWN_EVERY == 0
                      else zombie.PortraitZombie)
        enemy = enemy_type(
            x, y, self.head_group, can_swim=can_swim,
        )
        self.zombie_groups[map_y].add(enemy)
        return enemy

    # 能否种植物的判断：
    # 先判断位置是否合法 isValid(map_x, map_y)
    # 再判断位置是否可用 isMovable(map_x, map_y)
    def canSeedPlant(self, plant_name, mouse_pos=None):
        x, y = pg.mouse.get_pos() if mouse_pos is None else mouse_pos
        return self.map.checkPlantToSeed(x, y, plant_name)

    # 种植物
    def addPlant(self, mouse_pos=None):
        pos = self.canSeedPlant(self.plant_name, mouse_pos)
        if pos is None:
            return
        ...
        # A fast click may precede the next hover update. Refresh at the click,
        # not at the previous preview tile or the mouse's current position.
        self.setupHintImage(mouse_pos)
        ...
        match self.plant_name:
            ...
            case c.GASSUNFLOWER:
                new_plant = plant.GasSunFlower(
                    x, y, self.sun_group, self.bullet_groups[map_y]
                )
            case c.PEASHOOTER:
                new_plant = plant.PeaShooter(x, y, self.bullet_groups[map_y])
            case c.GRINDEVOURER:
                new_plant = plant.GrinDevourer(x, y, self)
            case c.HEADPHONEBOXER:
                new_plant = plant.HeadphoneBoxer(x, y, self, map_y)
            ...
            case c.PORTRAITCHERRYBOMB:
                new_plant = plant.PortraitCherryBomb(x, y, self.bullet_groups[map_y])
            ...
            case c.PORTRAITPOTATOMINE:
                new_plant = plant.PortraitPotatoMine(x, y, self.bullet_groups[map_y])
            case c.SQUASH:
                new_plant = plant.Squash(
                    x, y, self.map.map[map_y][map_x][c.MAP_PLANT]
                )
            case c.PORTRAITSQUASH:
                new_plant = plant.PortraitSquash(x, y, self, map_y)
            case c.PORTRAITHEALER:
                new_plant = plant.PortraitHealer(x, y, self)
            case c.PORTRAITCHOMPER:
            ...
            case c.PORTRAITTALLNUT:
                new_plant = plant.PortraitTallNut(x, y)
            case c.TANGLEKLEP:
    ...
    def removeMouseImagePlus(self):
        self.drag_shovel = False
        self.shovel_rect = self.shovel.get_rect(topleft=self.shovel_positon)
        ...
        for i in range(self.map_y_len):
            for bullet in self.bullet_groups[i]:
                ...
                if bullet.name == c.PORTRAIT_PEA:
                    ...
                    continue
                if bullet.name == c.GRIN_SEED:
                    # 追踪弹保留发射组，碰撞需检查全场而非仅该组对应行。
                    for group in self.zombie_groups:
                        for target in group:
                            if bullet.hits(target):
                                bullet.on_hit(target, self.bullet_groups[i])
                    continue
                    ...
                    for zombie in self.zombie_groups[i]:
                        ...
                        if collided_func(zombie, bullet):
                            if zombie.state != c.DIE:
                                ...
                                bullet.setExplode(play_sound=False)
                                self.impact_audio.play(
                                    'fire' if bullet.name == c.BULLET_FIREBALL else 'pea',
                                    zombie.rect.centerx, self.game_info[c.SOUND_VOLUME],
                                )
                                # 火球有溅射伤害
        ...
        for i in range(self.map_y_len):
            for zombie in self.zombie_groups[i]:
                ...
                if target_plant:
                    ...
                    if target_plant.name in {c.PORTRAITTALLNUT, c.PORTRAITPOTATOMINE}:
                        # The tall portrait extends above its row; use its planted base.
                    ...
                    if zombie.name in {c.POLE_VAULTING_ZOMBIE} and (
                        ...
                        if target_plant.name == c.GIANTWALLNUT:
                            ...
                            if (
                                c.TALL_BARRIER_PLANTS.intersection(self.map.map[zombie.jump_map_y][
                                    zombie.jump_map_x
                                ][c.MAP_PLANT])
                            ):
    ...
    def checkPlant(self, target_plant, i):
        ...
        if not target_plant.attack_check:
            ...
            for zombie in self.zombie_groups[i]:
                if target_plant.canAttack(zombie):
                    ...
                    break
        elif target_plant.name == c.PORTRAITPOTATOMINE:
            for zombie in self.zombie_groups[i]:
                if target_plant.canAttack(zombie):
                    target_plant.setAttack(zombie)
                    break
            if target_plant.start_boom and not target_plant.boomed:
                for zombie in self.zombie_groups[i]:
                    if target_plant.hitsBlast(zombie):
            ...
            if target_plant.state == c.IDLE and can_attack:
                ...
                target_plant.setIdle()
        elif target_plant.name == c.GRINDEVOURER:
            can_attack = any(
                target_plant.canAttack(zombie)
        ...
        # 灰烬植物与寒冰菇
            ...
            if target_plant.start_boom and (not target_plant.boomed):
                ...
                if target_plant.name in {c.REDWALLNUTBOWLING, c.CHERRYBOMB, c.PORTRAITCHERRYBOMB}:
                    self.boomZombies(
                        target_plant.rect.centerx,
                        i,
    ...
    def checkGameState(self):
        if self.campaign:
            # A loss must never be turned into a reward by an empty wave queue.
            if self.checkLose():
                c.SOUND_LOSE.play()
                c.SOUND_SCREAM.play()
                self.next, self.done = c.GAME_LOSE, True
            elif self.checkVictory() and self.card_rewards is None:
                self.card_rewards = CardRewards(self.campaign.stage, self.guide_last_time)
                c.SOUND_WIN.play()
                self.removeMouseImage()
                self.drag_shovel = False
                self.card_tooltip.visible = False
            return
    ...
    def drawMouseShow(self, surface):
        if self.hint_plant:
            image, rect = plant_layout.image_and_rect(self.plant_name, self.hint_image, self.hint_rect)
            surface.blit(image, rect)
        x, y = pg.mouse.get_pos()
        self.mouse_rect.centerx = x
        self.mouse_rect.centery = y
        scale = plant_layout.scale_for(self.plant_name)
        if scale != 1:
            image = pg.transform.smoothscale(self.mouse_image,
                (round(self.mouse_rect.w * scale), round(self.mouse_rect.h * scale)))
    ...
    def drawMouseShowPlus(self, surface):   # 拖动铲子时的显示
        x, y = pg.mouse.get_pos()
        # Align the open mouth (rather than the forehead) to the click point.
        self.shovel_rect = self.shovel_open.get_rect(topleft=(x - 32, y - 68))
        # 铲子接近植物时会高亮提示
        map_x, map_y = self.map.getMapIndex(x, y)
        surface.blit(self.shovel_open, self.shovel_rect)
        if not self.map.isValid(map_x, map_y):
            return
        for i in self.plant_groups[map_y]:
            if plant_layout.visual_rect(i.name, i.image, i.rect).collidepoint(x, y):
    ...
    def showLevelProgress(self, surface, progress=None, num_flags=None):
        # Reuse the existing frame, fill, flag and zombie-head artwork.
        # Custom finite stages have 4/4/5/6 waves, not ten waves per flag.
        if progress is None:
            progress = self.wave_num / (self.map_data[c.NUM_FLAGS] * 10)
        progress = max(0, min(1, progress))
        if num_flags is None:
            num_flags = self.num_flags
        filled_width = int(150 * progress)
        # 画进度条框
        surface.blit(
    ...
    def draw(self, surface):
        ...
        # 以后可能需要插入一个预备的状态（预览显示僵尸、返回战场）
            ...
            if self.has_shovel:
                ...
                if not self.drag_shovel:
                    icon = self.shovel
                    if self.last_shovel_use is not None:
                        age = self.current_time - self.last_shovel_use
                        if 320 <= age < ShovelChew.DURATION:
                            frames = tool.GFX['shovel_chew_toolbar']
                            icon = frames[int((age - 320) / 110) % len(frames)]
                    surface.blit(icon, self.shovel_positon)
            # 画小菜单
            surface.blit(self.little_menu, self.little_menu_rect)

            self.menubar.draw(surface)
            for effect in self.heal_effects:
                if getattr(effect, 'ground_effect', False):
                    surface.blit(effect.image, effect.rect)
            for i in range(self.map_y_len):
                for item in self.plant_groups[i]:
                    plant_layout.draw(item, surface)
                self.zombie_groups[i].draw(surface)
                self.hypno_zombie_groups[i].draw(surface)
                for bullet in self.bullet_groups[i]:
                    plant_layout.draw_projectile(bullet, surface)
                self.drawZombieFreezeTrap(i, surface)
                if self.cars[i]:
                    self.cars[i].draw(surface)
            self.head_group.draw(surface)
            self.sun_group.draw(surface)
            self.shovel_effects.draw(surface)
            for effect in self.heal_effects:
                if not getattr(effect, 'ground_effect', False):
                    plant_layout.draw_heal_effect(effect, surface)

            if self.campaign:
                if self.campaign.total is None:
                    self.campaign.draw(surface, self.current_time)
                else:
                    self.showLevelProgress(surface,
                        progress=self.campaign.wave / self.campaign.total, num_flags=1)
